...
void DXUTUpdateStaticFrameStats()
{
    ...
    else
    {
		...
		//// Display - D3D11 <n> fps Vsync Off (<x>x<y>), R8G8B8A8_UNORM_SRGB (MS1, Q0) 
		//// n - frames/sec
  ...
  //                       strFmt, strMultiSample );
		...
		// Update display to - D3D11 <n> fps Vsync Off (<x>x<y>) 
		// n - frames/sec
		// x, y - Screen resolution
        swprintf_s( pstrStaticFrameStats, 256, L"D3D11 %%sVsync %s (%dx%d)",
                         ( pDeviceSettings->d3d11.SyncInterval == 0 ) ? L"off" : L"on",
                         pDeviceSettings->d3d11.sd.BufferDesc.Width, pDeviceSettings->d3d11.sd.BufferDesc.Height);
