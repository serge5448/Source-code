...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    accls.erase(std::remove_if(accls.begin(), accls.end(), [](accelerator& a) { return a.device_path == accelerator::cpu_accelerator; }), accls.end());
    std::wcout << std::endl << "Found " << accls.size() << " accelerator device(s) that are compatible with C++ AMP: " << std::endl;
    int n = 0;
